 ...
 */
export async function getAuthenticatedUser(context: APIContext): Promise<User> {
  const userId = context.locals.userId;

  if (!userId) {
    throw new UnauthorizedErrorWithStatus(
      "Authentication required - middleware did not set userId",
    );
  }
