...
export class RateLimitError extends ApiError {
  constructor(retryAfter: number, message = 'Too many requests') {
    super(429, 'RATE_LIMITED', message, { retry_after: [retryAfter.toString()] });
  }
}
...
export class InvalidEnumValueError extends ApiError {
  constructor(field: string, validValues: string[], message = 'Invalid enum value') {
    super(400, 'INVALID_ENUM_VALUE', `${message}: ${field}. Valid values: ${validValues.join(', ')}`);
  }
}

export class MileageLowerThanPreviousError extends ApiError {
  constructor(currentMileage: number, previousMileage: number) {
    super(422, 'MILEAGE_LOWER_THAN_PREVIOUS', 
...
export class CannotDeleteServiceError extends ApiError {
  constructor(message = 'Service is referenced by active reminder and cannot be deleted') {
