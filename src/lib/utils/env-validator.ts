...
export function validateEnvironmentVariables(): ValidationResult {
  ...
  if (supabaseUrl && !isValidSupabaseUrl(supabaseUrl)) {
    errors.push('SUPABASE_URL format is invalid. Should be https://xxx.supabase.co');
  }

  // Validate Supabase keys format (basic check)
  const supabaseKey = import.meta.env.SUPABASE_KEY;
  if (supabaseKey && !isValidSupabaseKey(supabaseKey)) {
    errors.push('SUPABASE_KEY format appears to be invalid');
  }
...
function isValidSupabaseUrl(url: string): boolean {
  try {
    ...
    return (
      ...
      (parsedUrl.hostname.endsWith('.supabase.co') || parsedUrl.hostname.includes('localhost'))
    );
  } catch {
    return false;
  }
...
export function logEnvironmentValidation(result: ValidationResult): void {
  if (result.isValid) {
    ...
    console.error('   - SUPABASE_KEY: Your Supabase anon/public key');
    console.error('\n📋 Optional environment variables:');
    console.error('   - SUPABASE_SERVICE_ROLE_KEY: Service role key for admin operations');
    console.error('   - OPENWEATHER_API_KEY: OpenWeather API key for weather data');
    console.error('   - OPENROUTER_API_KEY: OpenRouter API key for AI features');
  }
}
...
export function getEnvironmentInfo(): Record<string, string> {
  ...
  return {
    ...
    SUPABASE_KEY: maskValue(import.meta.env.SUPABASE_KEY),
    SUPABASE_SERVICE_ROLE_KEY: maskValue(import.meta.env.SUPABASE_SERVICE_ROLE_KEY),
