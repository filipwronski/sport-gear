...
export class FeedbackValidator {
  ...
  static validateCreateFeedbackCommand(command: any): CreateFeedbackCommand {
    ...
    if (typeof command.temperature !== 'number' || command.temperature < -50 || command.temperature > 50) {
      errors.temperature = 'Temperature must be a number between -50 and 50';
    }

    // Validate feels_like (-50 to 50)
    if (typeof command.feels_like !== 'number' || command.feels_like < -50 || command.feels_like > 50) {
      errors.feels_like = 'Feels like temperature must be a number between -50 and 50';
    }
    ...
    if (typeof command.humidity !== 'number' || command.humidity < 0 || command.humidity > 100) {
      errors.humidity = 'Humidity must be a number between 0 and 100';
    }

    // Validate rain_mm (optional, >= 0)
    if (command.rain_mm !== undefined && (typeof command.rain_mm !== 'number' || command.rain_mm < 0)) {
    ...
    if (typeof command.duration_minutes !== 'number' || command.duration_minutes <= 0) {
    ...
    if (typeof command.overall_rating !== 'number' || command.overall_rating < 1 || command.overall_rating > 5) {
      errors.overall_rating = 'Overall rating must be a number between 1 and 5';
    }

    // Validate zone_ratings (optional, values 1-5)
    if (command.zone_ratings !== undefined && !this.validateZoneRatings(command.zone_ratings)) {
      errors.zone_ratings = 'Invalid zone ratings - all values must be between 1 and 5';
    }
    ...
    if (command.shared_with_community !== undefined && typeof command.shared_with_community !== 'boolean') {
  ...
  static validateOutfitStructure(outfit: any): outfit is OutfitDTO {
    ...
    if (!VALID_TORSO_BASE.includes(outfit.torso.base) ||
    ...
    if (!VALID_SOCKS.includes(outfit.feet.socks) ||
  ...
  static validateZoneRatings(ratings: any): ratings is ZoneRatings {
    ...
    const validZones = ['head', 'torso', 'arms', 'hands', 'legs', 'feet', 'neck'];
