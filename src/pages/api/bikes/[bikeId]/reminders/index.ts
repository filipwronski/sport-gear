...
export const GET: APIRoute = async ({ request, locals, params }) => {
  try {
    ...
    // Apply sorting
    const sortField = queryParams.sort?.includes('km_remaining') ? 'triggered_at_mileage' : 'created_at';
...
export const POST: APIRoute = async ({ request, locals, params }) => {
  try {
    ...
    const validationResult = ReminderValidator.validateCreateReminderCommand(body);
