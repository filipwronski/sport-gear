...
export class RecommendationService {
  ...
  private calculateAdjustedTemperature(
    ...
    const activityAdjustment = rules.ACTIVITY_TEMP_ADJUSTMENTS[activityType] || 0;
  ...
  private calculateFeet(temp: number, rain: number, prefs: ThermalPreferences | null) {
    ...
    if (rain > 3) {
      ...
      covers = this.findMatchingRule(rules.FEET_COVERS_RULES, temp, { coldFeet });
    }
   ...
   */
  private findMatchingRule(rules: any[], temp: number, context: RuleContext = {}): string {
    for (const rule of rules) {
      ...
      if (rule.windThreshold && context.wind && context.wind < rule.windThreshold) {
        continue;
      }
      
      // Check rain threshold (special handling)
      if (rule.rainThreshold && context.rain && context.rain <= rule.rainThreshold) {
        continue;
      }
