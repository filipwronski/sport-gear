...
export class RecommendationWeatherService {
   ...
   */
  async getWeather(locationId: string, userId: string, date?: string): Promise<WeatherDTO> {
   ...
   */
  private async getLocationCoordinates(locationId: string, userId: string): Promise<Coordinates> {
    // TEMPORARY: For demo purposes, return hardcoded Warsaw coordinates
    // In production, this would query the database properly
    if (import.meta.env.DEV && locationId === '94fd4d7a-2fdd-4bd7-949e-4befdcdb7032') {
      console.log('[RecommendationWeatherService] Using hardcoded Warsaw coordinates for demo');
      return {
        longitude: 21.0122,
        latitude: 52.2297
      };
    ...
    if (error || !data) {
      console.error('[RecommendationWeatherService] Location query failed:', error);
  ...
  private async getCachedWeather(cacheKey: string): Promise<WeatherDTO | null> {
    try {
      ...
      console.error('[RecommendationWeatherService] Cache lookup failed:', error);
   ...
   * Uses service client in development to bypass RLS
   */
  private async setCachedWeather(cacheKey: string, data: WeatherDTO, ttlSeconds: number): Promise<void> {
    try {
      // Use service client in development to bypass RLS issues
      const client = import.meta.env.DEV ? supabaseServiceClient : supabaseClient;
      ...
      if (error) {
        console.error('[RecommendationWeatherService] Failed to cache weather:', error);
        // Don't throw - caching failure shouldn't break the API response
      }
    } catch (error) {
      console.error('[RecommendationWeatherService] Cache operation failed:', error);
